...
describe List, type: :model do
  ...
  it 'Shows false, when no owner' do
    user_lists = nil
    allow(list.user_lists).to receive(:where).with(user_id: user.id, owner: true).and_return(user_lists)

    is_owner = list.is_owner?(user)
  ...
  it '50 percent done when 2/4 done todo_items' do
    ...
    allow(list.todo_items).to receive_message_chain(:done).and_return(done_items)
